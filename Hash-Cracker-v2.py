...
class hashCracking:
    ...
    def hashCrackWordlist(self, userHash, hashType, verbose, bruteForce=False):
        start = time.time()
        solved = False
        self.lineCount = 0
        if "md5" in hashType:
        ...
        if bruteForce is True:
            ...
            files = [os.path.join(wordlist_folder, f) for f in os.listdir(wordlist_folder) if f.endswith('.txt')]
            for wordlist_file in files:
                print(f"[*] Searching in {wordlist_file}")
...
def main(argv):
    ...
    print("[Running on %s]\n" % checkOS())
    try:
        opts, args = getopt.getopt(argv, "ih:t:nv", ["ifile=", "ofile="])
    except getopt.GetoptError:
        print('[*] ./Hash-Cracker.py -t <type> -h <hash>')
    ...
    with open('SavedHashes.txt', 'a+') as savedHashFile:
        for solvedHash in savedHashFile:
            ...
            try:
                h = hashCracking()
                h.hashCrackWordlist(userHash, hashType, verbose, bruteForce=numbersBruteForce)
                ...
                print("\n[-] Couldn't find wordlist folder or files")
                print("[*] Is this right?")
                print("[>] wordlist/")
